...
public class LegisterServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (result && !(legisterUser.getName()).equals("") && !(legisterUser.getPass().equals(""))) {//同名のnameが存在しなかった場合の処理

			dispatcher = request.getRequestDispatcher("/WEB-INF/jsp/legisterCheck.jsp");

		} else if (result && (legisterUser.getName()).equals("") && !(legisterUser.getPass().equals(""))) {
			request.setAttribute("errorMsg", "名前を入力してください");
			dispatcher = request.getRequestDispatcher("/WEB-INF/jsp/legister.jsp");
		} else if (result && !(legisterUser.getName()).equals("") && (legisterUser.getPass().equals(""))) {
			request.setAttribute("errorMsg", "パスワードを入力してください");
			dispatcher = request.getRequestDispatcher("/WEB-INF/jsp/legister.jsp");
		} else if (result && (legisterUser.getName()).equals("") && (legisterUser.getPass().equals(""))) {
			request.setAttribute("errorMsg", "項目を入力してください");
			dispatcher = request.getRequestDispatcher("/WEB-INF/jsp/legister.jsp");
